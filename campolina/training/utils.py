...
def save_model(model: nn.Module, epoch: int, step: int, val_loss: float, val_f1: float, val_mcc: float, name: str = None):
    path = f'weights/{model.name if name is None else name}'
    os.makedirs(path, exist_ok=True)
    torch.save(model.state_dict(), f'{path}/{mkname(epoch, step, val_loss, val_f1, val_mcc)}')
